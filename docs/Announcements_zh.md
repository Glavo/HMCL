...
分类使用非空字符串标识，可以增加新分类。
分类开关保存在 HMCL 目录下的 `state/launcher-state.json` 中，使用 `announcementCategoryVisibility` 对象记录。
对象的键为分类标识，值为布尔值：`true` 表示显示，`false` 表示隐藏。
没有记录的分类使用客户端定义的该分类默认值；当前分类默认开启。删除分类对应的记录即恢复默认值。
关闭分类后，该分类的公告不在主页展示，也不弹窗；筛选对所有 `severity` 生效。
筛选不会新增展示或关闭记录，也不删除已有记录；分类重新开启后，按正常展示条件重新判断。
...
每条公告实际展示后，将其 ID 保存到 `launcher-state.json` 的 `shownAnnouncements` 集合，仅下载到缓存不算展示。
设置了 `requiresShown` 的公告，只有在其依赖的公告 ID 已存在于 `shownAnnouncements` 中时才可展示。
...
用户关闭公告后，将其 ID 保存到 `launcher-state.json` 的 `closedAnnouncements` 集合。
...
以下路径均相对于 HMCL 目录，文件分别放在 `state` 和 `cache` 子目录中。

`state/launcher-state.json` 保存分类显示偏好、展示记录和关闭记录，相关字段例如：

```json
{
  "announcementCategoryVisibility": {
    "promotion": false,
    "security": true
  },
  "shownAnnouncements": [
...
`cache/announcements.json` 保存公告内容缓存及刷新所需的时间信息，例如：
...
清理缓存只会触发重新获取公告，不影响分类显示偏好、展示记录和关闭记录。
