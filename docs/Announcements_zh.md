...
HMCL 从静态 JSON 文件获取公告，在主页或弹窗中展示，并在本地保存缓存和关闭记录。
...
父子关系仅支持一层，父公告和子公告均为 `board`，且必须位于同一列表中。
子公告紧跟父公告展示，同级子公告使用相同排序规则。
父公告被关闭、过期或撤回时，其子公告一并隐藏；单独关闭子公告不影响父公告。

用户关闭公告后，将其 ID 保存到本地 `closed` 集合。
关闭记录不随公告撤回或过期而删除，避免同一公告恢复后重复提醒。
...
- 将服务端返回的 `Last-Modified` 解析为 Unix 毫秒时间戳保存，下次请求时格式化为 GMT 时区的 HTTP 日期，通过 `If-Modified-Since` 发送。
...
`lastAttemptTime` 和 `lastModified` 均使用 Unix 毫秒时间戳，分别表示本地请求时间和服务端提供的文件修改时间；服务端未提供 `Last-Modified` 时省略 `lastModified`。
