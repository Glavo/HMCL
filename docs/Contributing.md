...
| `HMCL_DISTRIBUTION_HOME`    | `-Dhmcl.distribution.home=<path>`            | Specifies the distribution-level configuration folder of HMCL | `.hmcl` in the directory containing the HMCL JAR, or `./.hmcl` if the JAR path is unavailable            |                           |
