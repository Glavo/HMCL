...
/// Renders the clipped portion of a [Decorator] window inside its drop shadow.
///
/// This pane owns the themed background, page and floating-content layers, title bar, navigation
/// controls, and window buttons. The surrounding shadow and the window move and resize behavior
/// remain the responsibility of [DecoratorSkin].
@NotNullByDefault
final class DecoratorContent extends StackPane {
    ...
    /// Handles title-bar double clicks when the platform supports maximizing this window.
    private final @Nullable EventHandler<MouseEvent> onTitleBarDoubleClick;

    /// Handles dragging that begins in the title bar.
    private final EventHandler<MouseEvent> onTitleBarDragged;
    ...
    ///
    /// @param decorator the decorator represented by this pane
    /// @param onTitleBarDoubleClick the handler for supported title-bar double clicks, or `null`
    ///                              when double-click maximization is disabled
    /// @param onTitleBarDragged the handler for drags that begin in the title bar
    DecoratorContent(
            Decorator decorator,
            @Nullable EventHandler<MouseEvent> onTitleBarDoubleClick,
            EventHandler<MouseEvent> onTitleBarDragged) {
        this.decorator = decorator;
        this.onTitleBarDoubleClick = onTitleBarDoubleClick;
        this.onTitleBarDragged = onTitleBarDragged;
    ...
    /// Returns the minimum width imposed by the title container.
    ///
    /// @return the title container's minimum width
    double getTitleMinWidth() {
        return titleContainer.getMinWidth();
    }

    /// Returns the minimum height imposed by the title container.
    ///
    /// @return the title container's minimum height
    double getTitleMinHeight() {
    ...
    private Node createNavBar(
        ...
        center.setOnMouseDragged(onTitleBarDragged);
