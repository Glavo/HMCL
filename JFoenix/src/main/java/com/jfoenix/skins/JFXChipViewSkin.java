...
public class JFXChipViewSkin<T> extends SkinBase<JFXChipView<T>> {
    ...
    public JFXChipViewSkin(JFXChipView<T> control) {
        ...
        this.behavior = new ChipViewBehaviorBase<>(control);
    ...
    final static class ChipViewBehaviorBase<T> extends BehaviorBase<JFXChipView<T>> {
        ...
        private static final MethodHandle TRAVERSE;

        static {

            Class<? extends Enum<?>> traversalMethod = null;
            Enum<?> key = null;
            try {
                traversalMethod = (Class<Enum<?>>)
                        Class.forName("com.sun.javafx.scene.traversal.TraversalMethod");
                for (Enum<?> enumConstant : traversalMethod.getEnumConstants()) {
                    if (enumConstant.name().equals("KEY")) {
                        key = enumConstant;
                        break;
                    }
                }

                throw new AssertionError("com.sun.javafx.scene.traversal.TraversalMethod.KEY not found");
            ...
            try {
                ...
                if (key != null) {
                    TRAVERSE = MethodHandles.insertArguments(lookup.findStatic(FocusTraversalInputMap.class, "traverse",
                            MethodType.methodType(void.class, Node.class, Direction.class, traversalMethod)), 2, key);
                } else {
                    TRAVERSE = lookup.findStatic(FocusTraversalInputMap.class, "traverse",
                            MethodType.methodType(void.class, Node.class, Direction.class));
                }
            } catch (Throwable e) {
